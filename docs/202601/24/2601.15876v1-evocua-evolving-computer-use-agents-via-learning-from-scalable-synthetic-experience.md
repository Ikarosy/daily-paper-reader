...
**问题**：现有的计算机操作智能体（CUA）受限于静态数据规模，难以捕捉长程任务中的复杂因果动态。
**方法**：提出EvoCUA，通过可验证的合成引擎生成任务，并利用大规模异步沙盒演练与迭代进化学习策略，将数据生成与策略优化整合为自我持续的进化循环。
**结论**：EvoCUA在OSWorld基准测试中达到56.7%的成功率，刷新了开源模型的最优纪录，并证明了该进化范式在不同规模基础模型上的通用性与可扩展性。
